...
func readBuffForString_constant(sshOut io.Reader, prompt string) string {
  buf := make([]byte, 65535)
  waitingString := ""
  ...
  // fmt.Println("WS: ",waitingString[len(waitingString)-len(prompt):])

  for err == nil && waitingString[len(waitingString)-len(prompt):] != prompt {
...
func readBuffForString_inbound(sshOut io.Reader, prompt string) string {
  buf := make([]byte, 65535)
  waitingString := ""
  n, err := sshOut.Read(buf) //this reads the ssh terminal
  if err != nil {
