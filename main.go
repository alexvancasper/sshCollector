...
func main() {
  ...
  var commands []string
  var common_commands []string
  var archive_file string

  commands, _ = read_command_file(conf.Common.Command_file)

  for i:=0;i<len(commands);i++{
    common_commands = append(common_commands, "rtr:"+commands[i])
  }

  if conf.Version != version {
    log.Printf("Version of config file is %s this version is not compatible with current version of collector", conf.Version)
    log.Printf("Quit.")
    return
  }
